...
// Poll for gamepad input about ~4 times per gameboy frame (~240 times second)
var GAMEPAD_POLLING_INTERVAL = 1000 / 60 / 4;
var GAMEPAD_KEYMAP_STANDARD_STR = "standard";

// When gamepad.mapping reports "standard"
var GAMEPAD_KEYMAP_STANDARD = [{
  gb_key: "b",
  gp_button: 0,
  type: "button"
}, {
...
function bindTouchRestore() {
  window.addEventListener("touchstart", function (e) {
    ...
    controller.style.display = "none"; // Tiago aqui esconde o ecra contrallers
...
function bindKeyboard() {
  ...
  window.onkeyup = function (e) {
    if (e.key === "Dead") {
      ...
      ["right", "left", "up", "down", "a", "b", "select", "start"].forEach(function (key) {
        GameBoyKeyUp(key);
      });
    }
    if (keyBindings[e.key]) {
...
function gamepadCacheValues(gamepad) {
  ...
  for (var k = 0; k < gamepad.buttons.length; k++) {
    ...
    if (gp.buttons.last !== undefined) gp.buttons.changed[k] = gp.buttons.cur[k] != gp.buttons.last[k];
  }

  // Read Axes
  for (var _k = 0; _k < gamepad.axes.length; _k++) {
    // Decode each dpad axis into two buttons, one for each direction
    gp.axes.cur[_k * 2] = gamepad.axes[_k] < 0;
    gp.axes.cur[_k * 2 + 1] = gamepad.axes[_k] > 0;

    // Update state changed if not on first input pass
    if (gp.axes.last !== undefined) {
      gp.axes.changed[_k * 2] = gp.axes.cur[_k * 2] != gp.axes.last[_k * 2];
      gp.axes.changed[_k * 2 + 1] = gp.axes.cur[_k * 2 + 1] != gp.axes.last[_k * 2 + 1];
    }
  }
...
function gamepadGetCurrent() {
  ...
  if (gamepad) if (gamepad.connected) return gamepad;
  return undefined;
}
function gamepadUpdate() {
  ...
  if (gamepad !== undefined) {
    ...
    for (var _i = 0; _i < gp.keybinds.length; _i++) gamepadHandleButton(gp.keybinds[_i]);
  } else {
...
function gamepadStart(gamepad) {
  ...
  if (gamepad.mapping === GAMEPAD_KEYMAP_STANDARD_STR || gamepad.axes.length >= 2 && gamepad.buttons.length >= 4) {
    ...
    gp.timerID = setInterval(function () {
...
function gamepadStop() {
  ...
  if (gp.timerID !== undefined) clearInterval(gp.timerID);
...
function initGamePad() {
  if (isTouchEnabled) {
    controller.style.display = "none"; // Tiago HIDE screen controllers
    isTouchEnabled = false;
  }
